...
def load_data(data_dir, batch_size=32, val_split=0.2, test_split=0.1):
    ...
    # Paths for Barack Obama dataset
    deepfake_dir = os.path.join(data_dir, 'Barack_Obama', 'train', 'deepfake_img')
    original_dir = os.path.join(data_dir, 'Barack_Obama', 'train', 'original_img')
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, test_loader = load_data(data_dir, batch_size=32)
    
    # Train the model with Early Stopping
    model = train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=25, patience=5, device=device)
    
    # Save the trained model
    save_model(model, 'path/to/models/spectra_vgg16_model_updated.pth')
